...
public class EdgeDiagram {
     ...
     *
     * @param type   関係の種類
     * @param number 変更する関係の番号 <br> ここにおける番号とは、生成した順番を表します。
     ...
     *
     * @param type   関係の種類
     * @param number 削除する関係の番号 <br> ここにおける番号とは、生成した順番を表します。
     */
    ...
    /**
     * <p> 関係の内容のテキストを取得します </p>
     *
     * @param number 取得する関係の番号 <br> ここにおける番号とは、生成した順番を表します。
     * @return 関係の内容のテキスト <br> 存在しなかった場合は空文字を返します。
     */
    public String getEdgeContentText(int number) {
        if (getContentType(number) == ContentType.Generalization) {
            return "";
        } else {
            if (relations.size() > 0) {
    ...
    /**
     * <p> 関係先のノードのIDを設定します </p>
     *
     * @param type   関係の種類
     * @param number 設定する関係の番号 <br> ここにおける番号とは、生成した順番を表します。
     * @param id     設定するID
     */
     ...
     */
    public boolean hasRelationSourceNodeSelected() {
     ...
     */
    public RelationshipAttributeGraphic searchCurrentRelation(Point2D mousePoint) {
        RelationshipAttributeGraphic content = null;
    ...
    public Point2D calculateIntersectionPointLineAndEndNodeSide(Point2D startPoint, Point2D endPoint, double endNodeWidth, double endNodeHeight) {
        ...
        if (isHigherThatStartNode) {
            if (isLefterThatStartNode) {
                // 第二象限
                if (isIntersectedFromUpper)
                    point = util.calculateCrossPoint(startPoint, endPoint, upperLeft, upperRight);
            ...
            if (isLefterThatStartNode) {
                // 第三象限（X軸上に位置する場合を含む）
                if (isIntersectedFromBottom)
                    point = util.calculateCrossPoint(startPoint, endPoint, bottomLeft, bottomRight);
                else point = util.calculateCrossPoint(startPoint, endPoint, upperLeft, bottomLeft);
    ...
    public boolean isIntersectedFromUpperOrBottomSideInSecondNode(Point2D firstNode, Point2D secondNode, double secondNodeWidth, double secondNodeHeight) {
        ...
        if (util.isIntersected(firstNode, secondNode, upperLeft, upperRight) || util.isIntersected(firstNode, secondNode, bottomLeft, bottomRight))
            isIntersected = true;
